...
class SAC_REV(object):
        def __init__(self, obs_shape, action_shape, args):
                ...
                self.exp_actor_optimizer = torch.optim.Adam(
                        self.exp_actor.parameters(), lr=args.actor_lr, betas=(args.actor_beta, 0.999)
                )
        ...
        def update_actor_and_alpha(self, obs, latest_samples, L=None, step=None, update_alpha=True):
                ...
                for _ in range(self.iters):
                    ...
                    _, latest_pi, latest_log_pi, latest_log_std = self.actor(latest_obs, detach=True)
        ...
        def update_exp_actor(self, obs):
                self.exp_actor = deepcopy(self.actor)
                self.exp_actor_optimizer = torch.optim.Adam(
                        self.exp_actor.parameters(), lr=self.actor_lr, betas=(self.actor_beta, 0.999)
                )
                self.exp_actor_optimizer.load_state_dict(self.actor_optimizer.state_dict())
        ...
        def update(self, replay_buffer, L, step):
                ...
                if step % self.actor_update_freq == 0:
                        ...
                        self.update_actor_and_alpha(obs, latest_samples, L, step)
