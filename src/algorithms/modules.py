...
class Actor(nn.Module):
    def __init__(
        self, encoder, action_shape, hidden_dim, log_std_min, log_std_max
    ):
        super().__init__()
    ...
    def log_probs(self, states, actions, detach=False):
        ...
        mu, pi, log_pi = squash(mu, actions, log_pi)
...
class Critic(nn.Module):
    ...
    def get_grad(self, x, actions):
        # x = self.encoder(x, detach=False).requires_grad_(True)
        x = self.encoder(x, detach=False)
        ...
        grad_1 = grad_1.view(q1.shape[0], -1)
        grad_2 = grad_2.view(q2.shape[0], -1)
