...
def evaluate(env, agent, num_episodes, L, step, test_env=False):
    episode_rewards = []
...
def main(args):
    ...
    replay_buffer = utils.ReplayFeatBuffer(
    ...
    for step in range(start_step, args.train_steps + 1):
            ...
            if step % args.eval_freq == 0:
                ...
                if test_env is not None:
                    ...
                    if args.algorithm in [
                        ...
                        if step > 0:
                            agent.offline_update(replay_buffer, L, step)
                        exp_evaluate(
                            test_env,
                            agent,
                            args.eval_episodes,
        ...
        done_bool = (
            0 if episode_step + 1 == env._max_episode_steps else float(done)
        )
